...
export default function Navbar() {
  ...
  useEffect(() => {
    ...
    gsap.fromTo(navRef.current.children,
      { y: -30, opacity: 0 },
      { y: 0, opacity: 1, duration: 0.4, stagger: 0.04, ease: 'power3.out', delay: 0.1 }
    )
  }, [])
  ...
  return (
    <>
      ...
      <AnimatePresence>
        {open && (
          <motion.div initial={{ clipPath: 'circle(0% at calc(100% - 40px) 40px)' }} animate={{ clipPath: 'circle(150% at calc(100% - 40px) 40px)' }} exit={{ clipPath: 'circle(0% at calc(100% - 40px) 40px)' }} transition={{ duration: 0.35, ease: [0.76, 0, 0.24, 1] }} className="fixed inset-0 z-40 flex flex-col justify-end pb-20 px-8" style={{ background: 'var(--cream)' }}>
            <div className="space-y-2">
              {links.map((l, i) => (
                <motion.div key={l.href} initial={{ y: 40, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.1 + i * 0.03, duration: 0.3, ease: [0.76, 0, 0.24, 1] }}>
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.25 }} className="mt-12 flex gap-8">
