  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      gsap.utils.toArray('.lf').forEach((el) => {
        gsap.fromTo(el, { y: 50, opacity: 0 }, { y: 0, opacity: 1, duration: 1.2, ease: 'power3.out', scrollTrigger: { trigger: el, start: 'top 87%' } })
      })
      ...
      if (bigImg.current) {
        gsap.fromTo(bigImg.current,
          ...
          { clipPath: 'inset(0% 0% 0% 0%)', duration: 2, ease: 'expo.out', scrollTrigger: { trigger: bigImg.current, start: 'top 75%' } }
        )
      }
      ...
      gsap.utils.toArray('.l-img').forEach((el) => {
        gsap.fromTo(el, { y: 60, opacity: 0, scale: 0.95 }, { y: 0, opacity: 1, scale: 1, duration: 1.4, ease: 'power3.out', scrollTrigger: { trigger: el, start: 'top 85%' } })
      })
    }, ref.current)
    return () => ctx.revert()
  }, [])
