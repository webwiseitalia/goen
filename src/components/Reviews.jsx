  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      gsap.utils.toArray('.rf').forEach((el, i) => {
        ...
        const d = dirs[i % 4]
        gsap.fromTo(el, { x: d.x, y: d.y, opacity: 0, rotate: d.rotate }, { x: 0, y: 0, opacity: 1, rotate: 0, duration: 0.45 + i * 0.03, ease: 'power3.out', scrollTrigger: { trigger: el, start: 'top 90%' } })
      })
    }, ref.current)
    return () => ctx.revert()
  }, [])
